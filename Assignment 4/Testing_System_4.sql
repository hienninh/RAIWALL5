...
SELECT		EQ.QUESTION_ID, Q.CONTENT, count(EQ.exam_ID), GROUP_CONCAT(EQ.exam_ID SEPARATOR ';')
...
SELECT		Category_ID, COUNT(Q.Question_ID), GROUP_CONCAT(Q.CONTENT SEPARATOR ';')
FROM		Question Q
-- JOIN		categoryquestion CQ ON	CQ.CATEGORY_ID	=	Q.CATEGORY_ID
GROUP BY	Q.CATEGORY_ID;

-- Question 7: Thông kê mỗi Question được sử dụng trong bao nhiêu Exam
SELECT		QUESTION_ID,Count(EQ.exam_ID), Group_concat(EQ.exam_ID)
FROM		examquestion EQ
JOIN		exam E ON	EQ.exam_ID	=	E.exam_ID
GROUP BY	EQ.QUESTION_ID;

-- Question 8: Lấy ra Question có nhiều câu trả lời nhất
SELECT		AN.Question_ID, count(AN.Answer_ID) AS So_luong_Answer, GROUP_CONCAT(AN.Answer_ID)
FROM		Answer AN
...
WHERE	So_luong_Answer = (SELECT Max(So_luong_Answer) FROM	Answer_Question);
...
-- CTE
WITH		Account_Position AS
(SELECT 	P.POSITION_ID, POSITION_NAME, USER_NAME, count(ACCOUNT_ID) AS So_luong_Account
FROM		`Account` A
JOIN		Position P ON	A.POSITION_ID	=	P.POSITION_ID
...
WHERE	So_luong_Account = (SELECT MIN(So_luong_Account) FROM Account_Position);
