...
// not found entity (getByID not found)
	...
	protected ResponseEntity<Object> handleMissingServletRequestParameter(
			...
			WebRequest request) {
		ApiErrorResponse error = new ApiErrorResponse(HttpStatus.BAD_REQUEST, "Parameter not blank",
				exception.getMessage());
		return new ResponseEntity<>(error, error.getStatus());
	}
...
//	Error chuyền sai kiểu dữ liệu
	@Override
	protected ResponseEntity<Object> handleTypeMismatch(TypeMismatchException exception, HttpHeaders headers,
			HttpStatus status, WebRequest request) {
		ApiErrorResponse error = new ApiErrorResponse(HttpStatus.BAD_REQUEST, "Lỗi chuyền sai kiểu dữ liệu",
				exception.getMessage());
		return new ResponseEntity<Object>(error, error.getStatus());
	}
